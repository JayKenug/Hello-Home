...
patNum = 0;
swipeTips = "啊滑啊滑啊滑";
taskChooseList = ["淘宝喵币", "淘宝锤猫", "支付宝喵币", "京东", "设置脚本速度"];
 ...
 */
function taskChoose() {
    var options = dialogs.multiChoice("选择任务", taskChooseList);
    if (options == '') {
        toastLog("脚本已退出");
        exit();
    }
    ...
    if (options.indexOf(1) > -1) {
        ...
        toastLog("选择拍猫" + frequencyOptions[i] + "次,加随机数至" + patNum + "次");
    }
    runOptions(options);
}
